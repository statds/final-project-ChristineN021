---
title: "Proposal: A Comparison Between Classification Models Methods for Classifying Properties in the Hartford Real Estate Market As Etiher Affordable or Expensive"
...
The plan of action is to fit the public data with both a logistic regression model and a support vector machine model via the Python programming language. Establishing features, we would also make the target variable ’sale price’ for classification after setting a threshold for what would be considered affordable or expensive for the middle class. We split the data into a training and testing set the train the instantiated classifier with the training data to predict the target variable. The models will be evaluated and compared by looking at precision, recall, F1-score and a confusion table for both models (Musa, 2012). These methods would help establish a better model for classifying the direction of the real estate market through certain evaluation criteria.

## Discussion

I expect to find that the market will move in a positive manner, with prices increasing adjacently to the relative appraisal price as well as other features increasing in frequency. I also expect that the support vector machine model will more accurately fit and classify the data, however we have to consider that support vector machine models are not ideal for large data sets. We must also take into account that competition for each home, insurance rates, property accommodations like garages, gas or oil for home heating, location and etc may affect the final sale prices and offers.
...
If the results are not as expected, further studies must be done to evaluate what factors/features further affect the movement of the market, we must also then consider other confounding factors and why the logistic regression model is more accurate.

## Conclusion

This project intends to research and classify the movement of the real estate market and each homes affordability through a comparison between a logistic regressionmodel and a support vector machine model. We will be comparing the two models for accuracy in data fitting and prediction to hopefully ensure a more stable timeline or sought-out factors in the affordavble home purchasing process for consumers.
...
Wang, T., Li, Y.-Q., and Zhao, S.-F. (2008), “Application of SVM Based on Roguh Set in Real Real 
  Estate Prices Prediction,” Print ISBN 978-1-4244-2107-7.